# -*- coding: utf-8 -*-
# ============================================================================
#SKY 산공학술교류제 공급망 최적화 프로젝트 - 1단계: 전략적 입지 선정

#[목표]
# 7년(2018-2024)간의 총 수요를 기반으로, 총 비용(건설비 + 운송비)을 최소화하는 최적의 공장 및 창고 입지를 결정합니다.
# 혼합 정수 선형 계획법(MILP)을 사용하여 문제를 모델링하고 Gurobi로 최적해를 도출합니다.

#[핵심 로직]
# 1. 데이터 로드: 사이트 후보, 초기 건설 비용, 전체 기간 수요 데이터를 로드합니다.
# 2. 파라미터 설정: 후보지 집합(I, K, J), 거리 행렬, 운송 단가 등 최적화 모델에 필요한 파라미터를 계산합니다.
# 3. 최적화 모델 구성 (Gurobi):
#    - 결정 변수: 각 후보지의 건설 여부(Binary), 창고-도시 간 수요 할당량(Continuous), 공장-창고 간 물동량(Continuous)
#    - 제약 조건: 최대 시설 수, 도시별 시설 수 제한, 모든 수요 충족, 개설된 시설만 사용, 생산 능력 제약 등
#    - 목적 함수: 총 건설비 + 총 운송비 최소화
# 4. 결과 저장: 최적 입지 선정 결과를 CSV 파일로 저장합니다.
# ============================================================================
