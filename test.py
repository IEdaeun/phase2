# -*- coding: utf-8 -*-
# ============================================================================
#  STAGE 3 OPERATIONAL SIMULATOR (Major Fix & Enhancement)
# - 전술 계획(tactical_plan_advanced.json) 기반 일일 시뮬레이션
# - 주요 수정 사항:
#   (1) 운송비: PDF 규칙에 따라 컨테이너(4000unit) 단위로 산정
#   (2) 국경 통과비: '단위당'이 아닌 '운송 건당' 고정비로 수정
#   (3) 환경부담금: 생산/운송 CO2 배출량 및 환경세(톤당 올림) 추가
#   (4) 운송 모드 변경 수수료: 4주 모드 고정 및 변경 페널티 로직 구현
#   (5) 초기 건설비: 총 비용에 합산
#   (6) 제출 형식(DB): 일일 생산량 집계 후 모든 행에 동일하게 기록
#   (7) 기타: 코드 가독성 향상 및 상세 주석 추가
# ============================================================================
...
def main_stage3_simulator():
    print("--- 3단계 운영 시뮬레이션 시작 (오류 수정 및 기능 강화 버전) ---")
    ...
    for fac, g in cap_raw.groupby('factory'):
        ...
        for _, r in g2.iterrows():
            week_reg_cap[(fac, r['week'])] = r['reg_capacity']
            week_ot_cap[(fac, r['week'])] = r['ot_capacity']
    ...
    # 운송 모드 추적 및 비용 기록
    mode_tracker = {} # key=(from, to, weekday), value={'mode', 'lock_end_date'}
    transport_cost_tracker = defaultdict(lambda: deque(maxlen=28)) # key=(from, to, weekday), value=deque of daily costs
    ...
    for t_current in pd.date_range(DATE_START, DATE_END, freq='D'):
        ...
        # 매일 임시 저장소 초기화
        daily_prod.clear()
        daily_ship_fac_wh.clear()
        daily_ship_wh_city.clear()
        ...
        for fac, q in list(factory_orders.items()):
            ...
            while q and (reg_left_today > 1e-9 or ot_left > 1e-9):
                ...
                if tracker and tracker['lock_end_date'] >= t_current.date():
                    mode_used = tracker['mode'] # 잠금 기간에는 모드 강제
                ...
                co2_factor_truck = 0.40 # kg/km/container
        ...
        for fac, prods in daily_prod.items():
            ...
            # 해당 공장의 모든 선적 건에 동일한 총생산량 기록
            for ship in daily_ship_fac_wh:
                if ship['fac'] == fac:
                    db_rows.append({"date": t_current.strftime("%Y-%m-%d"), "factory/warehouse": fac, "sku": ship['sku'], "production_qty": total_reg, "ot_qty": total_ot, "ship_qty": int(round(ship['qty'])), "from": fac, "to": ship['wh'], "mode": ship['mode']})
        for ship in daily_ship_wh_city:
            db_rows.append({"date": t_current.strftime("%Y-%m-%d"), "factory/warehouse": ship['wh'], "sku": ship['sku'], "production_qty": 0, "ot_qty": 0, "ship_qty": int(round(ship['qty'])), "from": ship['wh'], "to": ship['city'], "mode": "TRUCK"})
        
        # 일일 DB 데이터베이스에 추가 (메모리 관리)
        if db_rows:
            submission_df_today = pd.DataFrame(db_rows)
            with sqlite3.connect(DB_NAME) as conn:
                submission_df_today.to_sql(TABLE_NAME, conn, if_exists="append", index=False)
