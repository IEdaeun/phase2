# =========================================================================================================
#SKY 산공학술교류제 공급망 최적화 프로젝트 - 2단계: 전술적 운영 규칙 수립

#[목표]
# 1단계에서 선정된 입지를 기반으로, 변화하는 수요에 효과적으로 대응하기 위한 중기적 운영 규칙(소싱 정책, 재고 정책)을 수립합니다.
# 데이터 분석 기반의 휴리스틱 접근법을 사용하여 현실적이고 강건한 계획을 도출합니다.

#[핵심 로직]
#1. 데이터 로드: 1단계 결과(선정된 공장/창고), 수요 데이터, 사이트 정보 등을 로드합니다.
#2. 소싱 정책 수립:
#    - 각 창고에 대해 모든 공장까지의 운송 비용(거리)을 계산하여 우선순위 목록을 생성합니다.
#    - 가장 저렴한 공장을 주 공급처(Primary), 두 번째로 저렴한 공장을 예비 공급처(Secondary)로 지정합니다.
#3. 재고 정책 수립 (ABC-XYZ 분석 기반):
#    - ABC 분석: 전체 수요량 기여도에 따라 SKU를 A(핵심), B(중간), C(기타) 등급으로 분류합니다.
#    - XYZ 분석: 수요 변동성(CV, 변동계수)에 따라 SKU를 X(안정), Y(보통), Z(불안정) 등급으로 분류합니다.
#    - 등급 조합(AX, AZ 등)에 따라 안전재고일수를 차등 적용하여, SKU 특성에 맞는 맞춤형 재고 정책을 수립합니다.
#    - 최종적으로 각 창고-SKU별 안전재고(SS)와 재주문점(ROP)을 계산합니다.
#4. 결과 저장: 수립된 소싱/재고 정책을 JSON 파일로 저장합니다.
# =========================================================================================================
# -*- coding: utf-8 -*-
